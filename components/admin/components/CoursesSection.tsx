...
import { Plus, BookOpen } from "lucide-react";
import { Spin, ConfirmModal } from "./ui";
import { CourseCard } from "./CourseCard";
import { CourseForm } from "./CourseForm";
import type { Course, CourseView } from "./types";
...
export function CoursesSection({
  ...
  const [confirmColor, setConfirmColor] = useState<"red" | "emerald">("red");
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      showToast("error", t("Network error", "خطأ في الاتصال"));
    } finally {
      setLoading(false);
    }
  }, []); // eslint-disable-line
  ...
  function askConfirm(opts: {
    ...
    setConfirmLabel(opts.label);
    setConfirmColor(opts.color);
  ...
  function togglePublish(c: Course) {
    askConfirm({
      title: c.published
        ? t("Unpublish Course", "إلغاء النشر")
        : t("Publish Course", "نشر الكورس"),
      message: c.published
        ? t(`Hide "${c.title}" from students?`, `إخفاء "${c.title}" عن الطلاب؟`)
        : t(`Publish "${c.title}" for students?`, `نشر "${c.title}" للطلاب؟`),
      label: c.published ? t("Unpublish", "إلغاء النشر") : t("Publish", "نشر"),
      color: c.published ? "red" : "emerald",
      action: async () => {
        const r = await fetch(`/api/admin/courses/${c.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
        ...
        if (r.ok) {
          ...
          const d = await r.json();
          showToast("error", d.error ?? t("Failed", "فشلت العملية"));
        }
      },
    });
  }
  ...
  function deleteCourse(c: Course) {
    askConfirm({
      title: t("Delete Course", "حذف الكورس"),
      message: t(
        `Delete "${c.title}"? This will also delete all lessons and enrollments.`,
        `حذف "${c.title}"؟ سيتم حذف كل الدروس والتسجيلات.`,
      ),
      label: t("Delete", "حذف"),
      color: "red",
      action: async () => {
  ...
  return (
    <div className="space-y-7">
      ...
      >
        ...
        <button
          onClick={() => setView("add")}
          className={`flex items-center gap-2.5 bg-[#0a2540] hover:bg-[#0d3060] text-white font-bold px-6 py-3.5 rounded-xl text-[14px] shadow-lg shadow-[#0a2540]/20 transition-all hover:-translate-y-0.5 ${isRTL ? "flex-row-reverse" : ""}`}
        >
          <Plus className="w-4 h-4" />
          {t("Add Course", "إضافة كورس")}
        </button>
