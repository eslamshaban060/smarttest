...
export function AdminDashboard({ currentUserId }: { currentUserId: string }) {
  ...
  const tabs = [
    ...
    { key: "users" as AdminTab, en: "Users", ar: "المستخدمون", Icon: Users },
    {
      key: "courses" as AdminTab,
      en: "Courses",
      ar: "الكورسات",
      Icon: BookOpen,
    },
    {
      key: "recharge" as AdminTab,
      en: "Recharge",
      ar: "طلبات الشحن",
      Icon: Wallet,
    },
    ...
    <div dir={isRTL ? "rtl" : "ltr"} className="min-h-screen bg-[#f4f6f9]">
      ...
      <header className="bg-primary sticky top-0 z-40 py-10 shadow-2xl">
        ...
        <div className="max-w-7xl mx-auto px-8 relative">
          ...
          {/* nav tabs */}
          <div
            className={`flex items-center gap-1 py-3 ${isRTL ? "flex-row-reverse" : ""}`}
          >
            {tabs.map((tb) => (
              <button
                key={tb.key}
                onClick={() => setActiveTab(tb.key)}
                className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-[13px] font-semibold transition-all ${isRTL ? "flex-row-reverse" : ""} ${
                  activeTab === tb.key
                    ? tb.key === "recharge"
      ...
      <main className="max-w-7xl mx-auto px-8 py-10">
        ...
        {activeTab === "recharge" && (
          <RechargeTab t={t} isRTL={isRTL} showToast={showToast} />
        )}
      </main>
    </div>
  );
